...
export type UpdateTranslationEntry = (key: string, locale: string, value: string) => Promise<void>;
export type DeleteTranslationEntry = (key: string) => Promise<void>;
...
export interface NavScreen {
    /** divkit_screen entry UUID — written verbatim into `?screen_id=`. */
    id: string;
    /** Human-readable label shown in the Navigate preset dropdown. */
    label: string;
}
...
export interface DivProEditorOptions {
    ...
    /**
     * Screens available as navigation targets for the "Navigate" action preset.
     * Populated by the onboarding flow editor with the flow's steps (screen
     * entry UUID + display label). When empty, the Navigate preset is hidden.
     */
    navScreens?: NavScreen[];
}
...
export interface DivProEditorInstance {
    ...
    refreshTanker(data: TankerMeta): void;
    destroy(): void;
}
...
export const DivProEditor = {
    init(opts: DivProEditorOptions): DivProEditorInstance {
        ...
        state.navScreens.set(opts.navScreens || []);
        ...
        // Accept the log_id from both the bare DivKit card (`json.card`) and the
        // wrapper format (`json.remote_layout.card`); otherwise wrapper entries
        // always fell back to the sample id below.
        const logId = json?.card?.log_id ?? json?.remote_layout?.card?.log_id;
        if (logId) {
        ...
        const app = mount(App, {
            ...
            props: {
                ...
                updateTranslationEntry: opts.api?.updateTranslationEntry,
                deleteTranslationEntry: opts.api?.deleteTranslationEntry,
                refreshTranslations: opts.api?.refreshTranslations
            }
        });
