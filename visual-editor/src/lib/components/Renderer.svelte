<script lang="ts">
    ...
    let langCodeVariable: AnyVariable | null = null;
    ...
    function rerender({
        ...
        if (instance) {
            ...
            if (langCodeVariable) {
                langCodeVariable.set(get(previewLanguageCode));
            }
            ...
            langCodeVariable = createVariable('language_code', 'string', get(previewLanguageCode));
            globalVariablesController.setVariable(langCodeVariable);

            for (const [varId, price] of Object.entries(get(previewProductPrices))) {
                if (varId && price) {
                    const priceVar = createVariable(varId, 'string', price);
                    globalVariablesController.setVariable(priceVar);
                    previewPriceVariables[varId] = priceVar;
                }
            }
    ...
    $: if (langCodeVariable) {
        langCodeVariable.set($previewLanguageCode);
    }

    $: {
        const prices = $previewProductPrices;
        for (const [varId, price] of Object.entries(prices)) {
            if (!varId) continue;
            if (previewPriceVariables[varId]) {
                previewPriceVariables[varId].set(price || '');
            } else if (globalVariablesController && price) {
                const priceVar = createVariable(varId, 'string', price);
                globalVariablesController.setVariable(priceVar);
                previewPriceVariables[varId] = priceVar;
            }
    ...
    $: {
        const _renderJson = ($divjsonStore.object.remote_layout ?? $divjsonStore.object) as DivJson;
        if (_renderJson.card?.states?.[0]?.div && rootPreview && $locale !== undefined) {
            rerender({
                divjson: _renderJson,
                tanker: $tanker,
                locale: $locale,
