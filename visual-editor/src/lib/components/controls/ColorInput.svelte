<script lang="ts">
    ...
    function onPaletteClick(): void {
        ...
        color2Dialog().show({
            ...
            paletteId: palId,
            showPalette: true,
            startWithPalette: true,
            target: elem,
            disabled: readOnly,
            direction: elem.getBoundingClientRect().right < innerWidth / 2 ? 'right' : 'left',
            callback(newValue, newPaletteId) {
                if (newPaletteId) {
                    value = paletteIdToValue(newPaletteId);
                } else {
                    value = newValue;
                }
...
>
    ...
    {#if hasDialog && $palette.length > 0}
        <button
            class="color-input__palette-btn"
            class:color-input__palette-btn_active={Boolean(paletteItem)}
...
<style>
    ...
    .color-input__palette-btn {
        flex: 0 0 auto;
        width: 28px;
        align-self: stretch;
        padding: 0;
        border: none;
        border-left: 1px solid var(--fill-transparent-3);
        border-right: 1px solid var(--fill-transparent-3);
        background: var(--fill-transparent-minus-1) no-repeat center url(../../../assets/palette.svg);
    ...
    .color-input__palette-btn:hover {
        background-color: var(--fill-transparent-1);
    }

    .color-input__palette-btn:focus-visible {
        outline: 1px solid var(--accent-purple);
        outline-offset: -2px;
    }

    .color-input__palette-btn_active {
        background-color: var(--accent-purple);
        filter: none;
    }

    .color-input__palette-btn_active:hover {
