...
import { productToJson, type JsonProduct, type Product } from './products';
import type { ActionDesc, EditorError, FileLimits, GetTranslationKey, Locale, NavScreen, SafeAreaEmulation, Source, TankerMeta, ValueFilters } from '../../lib';
...
export class State {
    ...
    screenId = writable<string>('');
    screenLabel = writable<string>('');
    screenType = writable<string>('');
    ...
    divjsonStore = derived(
        ...
        ([root, productsList, screenId, screenLabel, screenType]) => {
            ...
            const remoteLayout = this.toFullDivjson(usedTemplates, shortObject);

            const fullObject: Record<string, unknown> = {};
            if (screenId) fullObject.screen_id = screenId;
            if (screenType) fullObject.screen_type = screenType;
            if (screenLabel) fullObject.label = screenLabel;
            fullObject.remote_layout = remoteLayout;
            if (productsList.length) {
                fullObject.variables = productsList.map(productToJson);
            }
    ...
    setDivJson(json: any): void {
        ...
        // Support wrapper format { screen_id, screen_type, label, remote_layout, variables }
        if (json.remote_layout) {
            this.screenId.set(json.screen_id || '');
            this.screenLabel.set(json.label || '');
            this.screenType.set(json.screen_type || '');
            if (Array.isArray(json.variables)) {
                this.products.set(json.variables.map((it: any) => ({
                    ...it,
                    __id: this.genProductId()
                })));
                const initPrices: Record<string, string> = {};
                for (const it of json.variables) {
                    if (it.id) initPrices[it.id] = '0';
                }
                this.previewProductPrices.set(initPrices);
            } else {
                this.products.set([]);
                this.previewProductPrices.set({});
            }
            json = json.remote_layout;
        }
        ...
        if (Array.isArray(json.card?.variables)) {
            ...
            const dictVarNames = new Set<string>(
                json.card.variables
                    .filter((v: JsonVariable) => v.type === 'dict' && v.name !== 'local_palette')
                    .map((v: JsonVariable) => v.name)
            );

            const autoI18n = new Set<string>();

            // Auto-mark any dict variable with locale_ prefix
            dictVarNames.forEach(name => {
                if (name.startsWith('locale_')) autoI18n.add(name);
            });

            // Also scan expressions to catch any remaining dict vars used in i18n patterns
            const i18nPattern = /@\{[^}]*\b(\w+)\b/g;
            const scanForI18n = (val: unknown): void => {
                if (typeof val === 'string') {
                    let m: RegExpExecArray | null;
                    i18nPattern.lastIndex = 0;
                    while ((m = i18nPattern.exec(val)) !== null) {
                        if (dictVarNames.has(m[1])) autoI18n.add(m[1]);
                    }
                } else if (Array.isArray(val)) {
                    val.forEach(scanForI18n);
                } else if (val && typeof val === 'object') {
                    Object.values(val as Record<string, unknown>).forEach(scanForI18n);
                }
            };
            scanForI18n(json.card.states);
            this.i18nMarkedVars.set(autoI18n);
            ...
            this.i18nMarkedVars.set(new Set());
        }
    ...
    genProductId(): string {
        return `product${this.#productCounter++}`;
    }
